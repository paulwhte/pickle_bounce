...
function Sprite(scene, imageFile, width, height, z) {
	...
	if (!z) { //if z is not defined then it is set to 0
		z = 0;
	}
    this.z = z; 
	...
	this.setMoveAngle(this.moveAngle); //default direction is up (0 degrees);
...
function Sound(src) {
	...
	this.playSound = function (opt) {
		...
		if (typeof webkitAudioContext !== 'undefined') {
			...
			if(scene.audio_ctx.createGainNode){
				this.gain_node = scene.audio_ctx.createGainNode();
			} else {
				this.gain_node = scene.audio_ctx.createGain();	
			}
...
function Joy(threshold) {
    //virtual joystick for ipad
	//threshold dictates the number of pixels that must be moved for virtkeys to work
...
function TextBox(scene, image, width, height, z){
// A simplified text box that displays text within a box.
	tTextBox = new Sprite(scene, image, width, height, z);
	...
	tTextBox.setAlpha = function (alpha) {
		this.alpha = alpha;
		this.backAlpha = alpha;
	} // end setAlpha
    ...
    tTextBox.writeText = function (text, align) {
		...
		yOffset = this.borderSize + this.marginTop + this.fontSize + this.marginBottom;
		...
		if(this.textArray.length > 1){
			for(var i=0;i<this.textArray.length;i++){
				...
				if(ctx.textAlign == "left"){
					this.ctx.fillText(this.textArray[i], this.x + xOffset - this.width/2, this.y + yOffset - this.height/2 + this.fontSize/3 + this.fontSize * i);// + this.fontSize	
				}
			}
		} else {
			if(ctx.textAlign == "left"){
				this.ctx.fillText(this.textArray[0], this.x + xOffset - this.width/2, this.y + yOffset - this.fontSize/1.5);
			} else {
